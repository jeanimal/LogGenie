...
-- Updates existing Docker database to match current schema

-- Drop existing zscaler_logs table if it has old schema
DROP TABLE IF EXISTS zscaler_logs CASCADE;

-- Recreate zscaler_logs table with correct schema
...
-- Drop and recreate log_uploads table with correct schema
DROP TABLE IF EXISTS log_uploads CASCADE;
